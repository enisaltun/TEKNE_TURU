...
### 15.5 Kaynak Tur Kısıtlamaları

**Kaynak tur** FMR başlatabilmek için şu koşulları karşılamalı:
- `aktif_guests > 0` — Sıfır rezervasyonlu tur kaynak olamaz; aktarılacak kimse yoktur
- `aktif_guests < kapasite * 0.5` — Kapasite doluluk oranı %50'nin altında olmalı (birleştirmeye gerek yok sinyali)
- Kapasite 100% dolu tur kaynak OLAMAZ — zaten dolu tekneyi boşaltmak anlamsız

**Wizard & UI davranışı:** Birleştir butonu ve wizard step 1 listesi yalnızca bu koşulları sağlayan turları gösterir.

### 15.6 Hedef Tur Kısıtlamaları

**Hedef tur** FMR için seçilebilmek için:
- `aktif_guests < kapasite` — En az 1 boş yer olmalı; **%100 dolu tur hedef olamaz**
- Kaynak tur ile aynı kaptan olamaz — kaptanın kendi turunu kendisiyle birleştirmesi yasaktır
- Kaynak tur ile aynı id olamaz (otomatik engellenir)
- **Aynı bölge (`bolge`) zorunlu** — Farklı bölgedeki turlar birleştirilemez (ör. Bodrum turu ile Kaş turu birleştirilemez). Tekne birleştirme; aynı bölge, aynı kalkış noktası ve aynı tur içeriği çerçevesinde yapılabilir.
  - Teknik kontrol: `t.bolge === src.bolge` (her iki tarafın `bolge` alanı doluysa ve eşleşmiyorsa liste dışı)

**Kapasite uyum kontrolü (Step 3):** Kaynak + Hedef toplam kişisi hedef kapasiteyi aşarsa talep oluşturulamaz.

### 15.7 Tarih/Saat Çakışma Kuralı

- Birleştirme sırasında müşteri rezervasyonları kaynak turdan hedef tura taşınır
- Kaynak turdaki müşteriler aynı gün/saatte başka bir tura zaten kayıtlı olamaz — hedef tur tarihinin müşteri takvimiyle uyumlu olması beklenir
- **Kaptan aynı gün/saatte iki farklı tur yürütemez:** Birleştirme yapılacak hedef tur, kaynak kaptan için tarih çakışması yaratıyorsa FMR başlatılamaz (UI'da uyarı gösterilir)

### 15.8 FMR Aktifken Rezervasyon Tarih Değişikliği Yasağı

- Bir tur için aktif FMR varsa (`durum: 'onay_bekleniyor'` veya `'her_ikisi_kabul'`), o turdaki rezervasyonların tarih/saat alanı **değiştirilemez**
- Sebep: Transfer sürecindeki bir rezervasyonun tarihi değişirse müşteri farklı bir tura aktarılmış olur — bu tutarsızlık yaratır
- UI davranışı: Rezervasyon detay modalında FMR aktifse tarih alanı kilitli gösterilir ve uyarı banner'ı çıkar

---

## 16. Kapasite Aşımı Önleme Kuralları

### 16.1 Doluluk Hesaplama Prensibi

- Bir turun doluluk oranı **yalnızca aktif rezervasyonlar** üzerinden hesaplanır: `durum IN ('bekliyor', 'onaylandi')`
- `tamamlandi` (geçmiş kalkışlar) ve `iptal`/`reddedildi` rezervasyonlar **kesinlikle hesaba dahil edilmez**
- Kapasite (`kapasite` alanı) **tek bir kalkış için** geçerlidir — tüm tarihlerin toplamı değil
- Görsel tutarsızlık (ör. `16/8 kişi`) bu kuralın ihlalinden kaynaklanır

### 16.2 Rezervasyon Kabulünde Kapasite Kontrolü

- Yeni rezervasyon oluşturulurken veya onaylanırken: `aktif_guests + yeni_rezervasyon_guests ≤ kapasite` şartı sağlanmalı
- Kontrol kapsamı: aynı `tourId` + aynı `tarihISO` kombinasyonu
- Şart sağlanmıyorsa:
  - **Müşteri tarafı:** Rezervasyon formu kapalı gösterilir, "Tur dolu" mesajı çıkar
  - **Admin tarafı:** Onay butonu devre dışı, uyarı banner'ı gösterilir
  - **Sistem:** Otomatik onaylama akışında rezervasyon `bekliyor` durumunda kalır, otomatik `onaylandi`'ya geçmez

### 16.3 Overbooking Tespiti ve Yönetimi

- **Tespit:** `aktif_guests > kapasite` olan tur "kapasite aşımı" durumundadır
- **Görsel işaret:** Kaptan detay modalında AKTİF TURLAR listesinde `⚠ Aşım` etiketi gösterilir
- **Risk paneli:** Overbooking tespit edilen her tur için Risk & Uyarılar bölümüne otomatik uyarı eklenir
- **Admin aksiyonu:** Fazla rezervasyonun iptali veya başka tura transferi (FMR) gerekir

### 16.4 Doluluk Renk Eşikleri

| Doluluk | Renk | Anlamı |
|---|---|---|
| %0–49 | Yeşil | Normal |
| %50–79 | Amber | İzle |
| %80–99 | Turuncu | Yaklaşan dolu |
| %100 | Kırmızı | Dolu — yeni rezervasyon kabul edilmez |
| >%100 | Kırmızı + `⚠ Aşım` | Kapasite aşımı — acil müdahale gerekir |

---

*Son güncelleme: 2026-04-27 | Kural sayısı: 16 bölüm (16.4'e kadar)*
